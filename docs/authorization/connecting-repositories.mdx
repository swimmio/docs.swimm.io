---
id: Connecting Repositories
slug: connecting-repositories
sidebar_position: 3
---
import useBaseUrl from '@docusaurus/useBaseUrl';
import Link from '@docusaurus/Link';

# Connecting Repositories To workspaces

Once you've authenticated and created your first <Link to="/docs/creating-content/workspaces">workspace</Link>,
you'll need to add some repositories that you want to document.

From the main workspace menu, on the left side, click the plus sign next to "REPOS":

<img alt="Swimm Add Repo" src={useBaseUrl('/img/add-repo-1.png')} />

Next, a dialog will open allowing you to select from public, private and organizational 
repositories at Github:


<img alt="Swimm Add Repo" src={useBaseUrl('/img/add-repo-2.png')} />


Just select the repositories you wish to add. Note, if you have dozens or more repositories,
it might take a few seconds for the list to load. 

If you don't see the repository you wish to add, it's because Swimm doesn't have the 
correct <Link to="/docs/authorization/github-auth">permissions</Link>. You can fix that
by clicking the button at the bottom of the dialog:


<img alt="Swimm Add Repo" src={useBaseUrl('/img/add-repo-3.png')} />

If you still can't find the repository after doing the steps above, please reach out to
support.